...
function humanDate(value, locale) {
  ...
  return new Intl.DateTimeFormat(locale === "en" ? "en-GB" : "nl-NL", { day: "numeric", month: "long", year: "numeric", timeZone: "UTC" }).format(date);
}
...
export async function generateQuotePdf({ quoteNumber, snapshot, contact }) {
  ...
  y -= 10;
  text(en ? "AI that works for you, without you having to understand AI." : "AI die voor u werkt, zonder dat u AI hoeft te begrijpen.", { font: timesBold, size: 27, leading: 31 }); y -= 8;
  text(en ? `Issued ${humanDate(snapshot.issueDate, "en")} · valid through ${humanDate(snapshot.validUntil, "en")}` : `Uitgegeven ${humanDate(snapshot.issueDate, "nl")} · geldig t/m ${humanDate(snapshot.validUntil, "nl")}`, { size: 9, color: MUTED });
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const identity = `${AIOW_COMPANY.legalName} · ${AIOW_COMPANY.streetAddress} · ${AIOW_COMPANY.postalCode} ${AIOW_COMPANY.locality} · KvK ${AIOW_COMPANY.chamberOfCommerce}`;
    current.drawText(safeText(identity, helvetica), { x: MARGIN, y: 28, size: 7, font: helvetica, color: MUTED });
  }
