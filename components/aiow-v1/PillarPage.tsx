...
export function PillarPage({ data }: { data: Pillar }) {
  ...
  </main><footer className={shared.footer}><Link href="/" className={shared.logo}><span>AIOW</span><i /></Link><p>AI Operating Workflows</p><div><Link href="/tarieven">Tarieven</Link><a href="/privacy">Privacy</a><a href="/llms.txt">llms.txt</a></div></footer><BookingModal open={booking} onClose={() => setBooking(false)} returnFocus={bookingTrigger.current} /></div>;
}
