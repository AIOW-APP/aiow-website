...
export function PillarPage({ data, locale = "nl" }: { data: Pillar; locale?: AiowLocale }) {
  ...
  return <div className={`${shared.site} ${styles.pillarSite}`}><PublicHeader locale={locale} onBook={openBooking} /><main>
    ...
    <header className={styles.hero}><p className={shared.eyebrow}>{data.eyebrow}</p><h1>{data.title}</h1><p className={styles.answer}>{data.answer}</p><p className={styles.intro}>{data.introduction}</p><button className={shared.primaryButton} onClick={openBooking}>{en ? "Book a scan" : "Plan een scan"}</button></header>
