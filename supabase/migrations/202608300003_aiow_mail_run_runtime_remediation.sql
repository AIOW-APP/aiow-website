...
-- transferring object ownership. Add a separate self-granted SET-capable
-- membership only when required, then revoke that same grantor row before commit.
create temporary table aiow_temporary_role_memberships(
  role_name name primary key
) on commit drop;
create temporary table aiow_temporary_schema_create_privileges(
  role_name name primary key
) on commit drop;
...
begin
  ...
  loop
    if not pg_has_role(current_user,v_role,'SET') then
      ...
      execute format('grant %I to %I with admin false, inherit false, set true',v_role,current_user);
    end if;
  end loop;
end $temporary_owner_membership$;
do $temporary_schema_create$
declare
  v_role name;
begin
  foreach v_role in array array['aiow_mail_run_receipt_owner'::name,'aiow_mail_runtime_reader'::name]
  loop
    if not has_schema_privilege(v_role,'public','CREATE') then
      insert into aiow_temporary_schema_create_privileges(role_name) values(v_role);
      execute format('grant create on schema public to %I',v_role);
    end if;
  end loop;
end $temporary_schema_create$;
...
declare
  v_role name;
begin
  for v_role in select role_name from aiow_temporary_schema_create_privileges
  loop
