...
function downloadIcs(form: Form, requestId: string) {
  ...
  const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//AIOW//Scan booking//NL", "CALSCALE:GREGORIAN", "BEGIN:VEVENT", `UID:${requestId}@aiow.ai`, `DTSTAMP:${stamp}`, `DTSTART;TZID=Europe/Amsterdam:${compactDate}T${String(hour).padStart(2, "0")}${String(minute).padStart(2, "0")}00`, `DTEND;TZID=Europe/Amsterdam:${compactDate}T${String(Math.floor(end / 60)).padStart(2, "0")}${String(end % 60).padStart(2, "0")}00`, `SUMMARY:${escapeIcs("AIOW kennismaking — " + form.subject)}`, "DESCRIPTION:Boekingsaanvraag door AIOW ontvangen. De definitieve praktische afstemming volgt rechtstreeks.", "END:VEVENT", "END:VCALENDAR"];
...
export function BookingModal({ open, onClose, locale = "nl", returnFocus }: { open: boolean; onClose: () => void; locale?: "nl" | "en"; returnFocus?: HTMLElement | null }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const body = await response.json(); if (!response.ok || !body.ok) { setErrors(body.fields || {}); throw new Error(body.error || "failed"); }
      setRequestId(body.requestId); setStatus("success");
  ...
  return <div className={styles.modalBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget && !sending.current) onClose(); }}>
    <div className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="booking-title" tabIndex={-1} ref={dialog}>
      ...
      {status === "success" ? <div className={styles.success}><span className={styles.successMark}>✓</span><p className={styles.eyebrow}>{en ? "Request accepted" : "Aanvraag ontvangen"}</p><h2 id="booking-title">{en ? "Your scan request is registered." : "Je scanaanvraag staat geregistreerd."}</h2><p>{en ? "This calendar file is generated on your device. Practical confirmation follows separately." : "Het kalenderbestand wordt alleen op je apparaat gemaakt. Praktische bevestiging volgt apart."}</p><button type="button" className={styles.primaryButton} onClick={() => downloadIcs(form, requestId)}>{en ? "Download calendar file" : "Download kalenderbestand"}</button></div> : <form onSubmit={submit}>
        <p className={styles.eyebrow}>{en ? `Step ${step} of 3` : `Stap ${step} van 3`}</p><h2 id="booking-title">{en ? "Book a practical scan" : "Plan een praktische scan"}</h2>
