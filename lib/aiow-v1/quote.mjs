...
export function buildQuoteMailContent({ quoteNumber, snapshot, contact, source, country = "", receivedAt }) {
  ...
  return freeze({
    customerMail: { from: AIOW_COMPANY.publicEmail, to: [contact.email], subject: customerSubject, text: customerText, html: textToSafeHtml(customerText, snapshot.bookingUrl, en ? "en" : "nl") },
    internalMail: { from: AIOW_COMPANY.publicEmail, to: [AIOW_COMPANY.publicEmail], subject: internalSubject, text: internalText, html: textToSafeHtml(internalText) },
  });
}
