...
const [trust, nl, en, locale, sitemap, llms, header, footer, seo] = await Promise.all([
...
test("trust pages publish only the verified company facts and honest scope boundary", () => {
  for (const fact of ["AIOW B.V.", "71887466", "info@aiow.io", "Bijlmermeerstraat 30", "2131 HC", "Hoofddorp", "Nederland", "Netherlands"]) assert.match(trust, new RegExp(fact.replace(".", "\\.")));
...
test("trust layer exposes closed Organization schema and llms facts", () => {
  ...
  assert.match(seo, /export function organizationNode/); assert.match(seo, /"@id": `\$\{SITE_URL\}\/\#organization`/); assert.match(seo, /legalName: "AIOW B\.V\."/); assert.match(seo, /propertyID: "KvK"/); assert.match(seo, /"@type": "Country"/);
