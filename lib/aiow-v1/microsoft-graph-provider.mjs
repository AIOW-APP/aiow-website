...
export const AIOW_MAIL_SENDER = AIOW_COMPANY.transactionalEmail;
...
const BOOKING_TRUTH = Object.freeze({
  nl: Object.freeze({ received:"We hebben uw voorkeursdatum en -tijd ontvangen.", follows:"Een medewerker bevestigt de afspraak afzonderlijk." }),
  en: Object.freeze({ received:"We have received your preferred date and time.", follows:"A member of our team will confirm the appointment separately." }),
});
const BOOKING_CONFIRMATION_CLAIM = /\b(?:confirm(?:ed|s|ing)?|booked|bevestig\w*|ingepland)\b|\bafspraak\s+staat\s+vast\b/iu;
function htmlEscape(value) { return value.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;"); }
function validTemplateField(value, max) { return typeof value === "string" && value.length >= 1 && value.length <= max && !/[\r\n\0]/.test(value); }
function validCustomerBookingTruth(job) {
  return Object.values(BOOKING_TRUTH).some(({received,follows}) => {
    if (!(job.text.includes(received) && job.text.includes(follows) && job.html.includes(received) && job.html.includes(follows))) return false;
    const remainder = `${job.subject}\n${job.text}\n${job.html}`.replaceAll(received, "").replaceAll(follows, "");
    return !BOOKING_CONFIRMATION_CLAIM.test(remainder);
  });
}
export function buildCustomerBookingTemplateV2({ locale, customerName, preference }) {
  const truth = BOOKING_TRUTH[locale];
  if (!truth || !validTemplateField(customerName, 100) || !validTemplateField(preference, 200)) throw new TypeError("invalid customer booking template input");
  const subject = locale === "nl" ? "Uw afspraakvoorkeur is ontvangen" : "Your appointment preference has been received";
  const greeting = locale === "nl" ? `Beste ${customerName},` : `Dear ${customerName},`;
  const preferenceLine = locale === "nl" ? `Uw voorkeur: ${preference}` : `Your preference: ${preference}`;
  const text = `${greeting}\n\n${truth.received}\n${preferenceLine}\n${truth.follows}`;
  const html = `<p>${htmlEscape(greeting)}</p><p>${truth.received}</p><p>${htmlEscape(preferenceLine)}</p><p>${truth.follows}</p>`;
...
export function validateMailJobV2(job) {
  ...
  if (job.kind === "customer_booking" && (job.attachments.length !== 0 || !validCustomerBookingTruth(job))) return false;
  return true;
}
...
function retryAfterSeconds(response, options) {
  const raw = response.headers.get("retry-after");
  if (!raw) return null;
  if (/^\d{1,5}$/.test(raw)) return Math.min(Math.max(Number(raw), 1), 3600);
  const target = Date.parse(raw), now = Number(options.now ? options.now() : Date.now());
  if (!Number.isFinite(target) || !Number.isFinite(now) || target <= now) return null;
  return Math.min(Math.max(Math.ceil((target - now) / 1000), 1), 3600);
}
...
export async function sendMicrosoftGraphJob(job, gate, options = {}) {
  ...
  if (!tokenResponse.ok) {
    const id = requestId(tokenResponse);
    if (tokenResponse.status === 429) {
      const seconds = retryAfterSeconds(tokenResponse, options);
    ...
    if (tokenResponse.status === 408) return transient(job, options, "timeout_before_response", 408, id);
    if (tokenResponse.status >= 500) return transient(job, options, "graph_5xx", tokenResponse.status, id);
    let oauthCode = ""; try { oauthCode = JSON.parse(await boundedText(tokenResponse))?.error ?? ""; } catch { /* status remains authoritative */ }
    if (["temporarily_unavailable","server_error"].includes(oauthCode)) return transient(job, options, "network_before_response", tokenResponse.status, id);
    return permanent(job, options, "oauth_authentication_failed", tokenResponse.status, id);
  }
  let token;
  try { token = JSON.parse(await boundedText(tokenResponse)); } catch { return transient(job, options, "network_before_response", tokenResponse.status, requestId(tokenResponse)); }
  if (!plain(token) || typeof token.access_token !== "string" || token.access_token.length < 8 || token.access_token.length > 8192 || token.token_type !== "Bearer") return transient(job, options, "network_before_response", tokenResponse.status, requestId(tokenResponse));
  if (!gateCurrent(gate, options)) return permanent(job, options, "exchange_rbac_denied", 403);
  ...
  if (graphResponse.status === 429) {
    const seconds = retryAfterSeconds(graphResponse, options);
    if (seconds !== null && typeof options.onRetryAfter === "function") options.onRetryAfter(seconds);
    return transient(job, options, "throttled_429", 429, id);
  }
