...
const HOME_CONTEXT_SLUGS = new Set(["woning", "villa-signature"]);
...
export function QuoteModal({ open, onClose, locale = "nl", returnFocus, calculatorConfig }: { open: boolean; onClose: () => void; locale?: "nl" | "en"; returnFocus?: HTMLElement | null; calculatorConfig: CalculatorQuoteConfig | null }) {
  ...
  const contextOptions = useMemo(() => pricingContexts.filter((item) => calculatorConfig?.segment === "business" ? item.category === "business" : calculatorConfig?.segment === "home" ? HOME_CONTEXT_SLUGS.has(item.slug) : item.category === "building" && !HOME_CONTEXT_SLUGS.has(item.slug)), [calculatorConfig?.segment]);
  ...
  async function submit(event: FormEvent) {
    ...
    if (!form.name.trim()) clientErrors.name = en ? "Enter your name." : "Vul uw naam in.";
  ...
  const explicitError = errorKind === 429 ? (en ? "Too many attempts. Your input is preserved; wait and retry." : "Te veel pogingen. Uw invoer blijft staan; wacht even en probeer opnieuw.") : errorKind === 503 || errorKind === 502 ? (en ? "Your indication could not be recorded. No PDF was created and nothing was sent; your input is preserved." : "Uw indicatie kon niet worden vastgelegd. Er is geen PDF gemaakt en niets verzonden; uw invoer blijft staan.") : (en ? "Your indication could not be recorded. Check the fields or retry later; nothing was sent." : "Uw indicatie kon niet worden vastgelegd. Controleer de velden of probeer later; er is niets verzonden.");
  return <div className={styles.modalBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget && !sending.current) onClose(); }}>
    <div className={`${styles.modal} ${styles.quoteModal}`} role="dialog" aria-modal="true" aria-labelledby="quote-title" tabIndex={-1} ref={dialog}>
      ...
      {status === "success" ? <div className={styles.success}><span className={styles.successMark}>✓</span><p className={styles.eyebrow}>{en ? "Indication received" : "Indicatie ontvangen"}</p><h2 id="quote-title">{quoteNumber}</h2><p>{en ? "Your indication is ready. The PDF download has started and a copy is on its way to your email. The next step is a free half-hour scan with a person." : "Uw indicatie is klaar. De PDF-download is gestart en een kopie is onderweg naar uw e-mail. De volgende stap is een gratis scan van een half uur met een mens."}</p><button type="button" className={styles.primaryButton} onClick={onClose}>{en ? "Close" : "Sluiten"}</button></div> : <form onSubmit={submit} noValidate>
        <p className={styles.eyebrow}>{en ? "Quote indication" : "Offerte-indicatie"}</p><h2 id="quote-title">{en ? "Your configuration, clearly documented." : "Uw configuratie, helder vastgelegd."}</h2><p className={styles.quoteIntro}>{en ? "We recalculate your indication and only release the PDF when your request has been recorded. You receive the same PDF by email." : "We berekenen uw indicatie opnieuw en geven de PDF pas vrij wanneer uw aanvraag is vastgelegd. U ontvangt dezelfde PDF ook per e-mail."}</p>
        ...
        <div className={styles.quoteGrid}>
          <fieldset className={styles.quoteSection}><legend>{en ? "Context and Smart Design" : "Context en Smart Design"}</legend>
            ...
            {form.modules.length > 0 && <p className={styles.quoteIntro}>{calculatorConfig.segment === "business" ? (en ? "For this indication we estimate 12 m² per person, with a minimum of 25 m². We confirm the actual surface and any technology budget during the scan." : "Voor deze indicatie rekenen we met 12 m² per persoon, met minimaal 25 m². De werkelijke oppervlakte en een eventueel technologiebudget bevestigen we tijdens de scan.") : (en ? "Smart Design uses the surface entered in the calculator. Any technology budget is discussed during the scan and is not included in this indication." : "Smart Design gebruikt de oppervlakte uit de calculator. Een eventueel technologiebudget bespreken we tijdens de scan en staat niet in deze indicatie.")}</p>}
          </fieldset>
          <fieldset className={styles.quoteSection}><legend>{en ? "Contact" : "Contact"}</legend>
            <label>{en ? "Name" : "Naam"}<input id="quote-name" autoComplete="name" maxLength={100} required value={form.name} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "quote-name-error" : undefined} onChange={(event) => update("name", event.target.value)} />{errors.name && <small id="quote-name-error">{errors.name}</small>}</label><label>E-mail<input id="quote-email" type="email" autoComplete="email" maxLength={254} required value={form.email} aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "quote-email-error" : undefined} onChange={(event) => update("email", event.target.value)} />{errors.email && <small id="quote-email-error">{errors.email}</small>}</label><label>{en ? "Phone (for one short callback)" : "Telefoon (voor één korte terugbelafspraak)"}<input id="quote-phone" type="tel" autoComplete="tel" maxLength={40} required value={form.phone} aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "quote-phone-error" : undefined} onChange={(event) => update("phone", event.target.value)} />{errors.phone && <small id="quote-phone-error">{errors.phone}</small>}</label>
        ...
        <div className={styles.modalActions}><button type="button" className={styles.textButton} onClick={onClose} disabled={status === "sending"}>{en ? "Cancel" : "Annuleren"}</button><button type="submit" disabled={status === "sending"} className={styles.primaryButton}>{status === "sending" ? (en ? "Preparing…" : "Bezig…") : (en ? "Get your indication as PDF" : "Ontvang uw indicatie als PDF")}</button></div>
      </form>}
    </div>
  </div>;
}
