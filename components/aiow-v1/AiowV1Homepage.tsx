...
export function AiowV1Homepage({ locale = "nl" }: { locale?: "nl" | "en" }) {
  ...
  const solutions = en ? [
    ["AI automation", "Turn repeated knowledge work into controlled workflows with human checkpoints.", "/ai-automatisering"],
    ["Local AI", "Run sensitive AI workloads closer to your own data and infrastructure.", "/lokale-ai"],
    ["Smart Office", "Connect spaces, signals and workflows—implementation and hardware scoped separately.", "/smart-office"],
    ["AIOW Home", "A partner-dependent route for practical home intelligence, without pretending hardware is included.", "/home"],
  ] : [
  ...
  return <div className={styles.site}><PublicHeader locale={locale} onBook={openBooking} />
    <main>
      <section className={styles.hero}>
        <div className={styles.heroCopy}><p className={styles.eyebrow}>{en ? "AI, installed with precision" : "AI, precies geïnstalleerd"}</p><h1>{en ? <>Working AI for your <em>company and building.</em></> : <>Werkende AI voor je <em>bedrijf en gebouw.</em></>}</h1><p className={styles.lead}>{en ? "AIOW scopes, builds and manages practical AI systems. Start with a transparent indication, then establish the definitive scope in a scan." : "AIOW inventariseert, bouwt en beheert praktische AI-systemen. Begin met een transparante indicatie; bepaal daarna de definitieve scope in een scan."}</p><div className={styles.heroLinks}><button className={styles.primaryButton} onClick={openBooking}>{en ? "Book a scan" : "Plan een scan"}</button><a href={en ? "/en#solutions" : "/#oplossingen"}>{en ? "Explore routes" : "Bekijk routes"} ↓</a></div><dl className={styles.heroFacts}><div><dt>{en ? "Start" : "Vanaf"}</dt><dd>€ 4.950</dd></div><div><dt>{en ? "First step" : "Eerste stap"}</dt><dd>{en ? "Practical scan" : "Praktische scan"}</dd></div><div><dt>{en ? "Terms" : "Voorwaarden"}</dt><dd>{en ? "Published" : "Gepubliceerd"}</dd></div></dl></div>
      ...
      <section id={en ? "solutions" : "oplossingen"} className={styles.solutions}><div className={styles.sectionHead}><p className={styles.eyebrow}>{en ? "Solutions" : "Oplossingen"}</p><h2>{en ? "Four routes. One measured way of working." : "Vier routes. Eén beheerste werkwijze."}</h2><p>{en ? "Choose the environment where AI must become useful. Every route starts with boundaries and a real implementation scope." : "Kies de omgeving waarin AI nuttig moet worden. Elke route begint bij grenzen en een echte implementatiescope."}</p></div><div className={styles.solutionList}>{solutions.map(([title, body, href], index) => <a href={href} key={title} className={styles.solution}><span>0{index + 1}</span><div><h3>{title}</h3><p>{body}</p></div><b aria-hidden="true">↗</b></a>)}</div></section>
