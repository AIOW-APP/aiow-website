...
const BOOKING_CONFIRMATION_CLAIM = /\b(?:appointment|booking)\s+(?:is\s+|has\s+been\s+)?confirmed\b|\bconfirmed\s+(?:appointment|booking)\b|\bafspraak\s+(?:is\s+)?bevestigd\b|\bbevestigde\s+afspraak\b|\bdefinitief\s+ingepland\b|\bafspraak\s+staat\s+vast\b/iu;
...
function validCustomerBookingTruth(job) {
  ...
  if (BOOKING_CONFIRMATION_CLAIM.test(combined)) return false;
  return Object.values(BOOKING_TRUTH).some(({received,follows}) => job.text.includes(received) && job.text.includes(follows) && job.html.includes(received) && job.html.includes(follows));
}
...
export async function sendMicrosoftGraphJob(job, gate, options = {}) {
  ...
  if (!tokenResponse.ok) {
    ...
    if (tokenResponse.status === 429) {
      const rawRetryAfter = tokenResponse.headers.get("retry-after");
