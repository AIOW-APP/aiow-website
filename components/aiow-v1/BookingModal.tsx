  ...
  return <div className={styles.modalBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget && !sending.current) onClose(); }}>
    <div className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="booking-title" tabIndex={-1} ref={dialog}>
      ...
      {status === "success" ? <div className={styles.success} role="status"><span className={styles.successMark}>✓</span><p className={styles.eyebrow}>{en ? "Durable receipt" : "Duurzaam ontvangstbewijs"}</p><h2 id="booking-title">{en ? "Your preferred scan request was received." : "Je voorkeur voor een scan is ontvangen."}</h2><p>{en ? "This does not reserve provider calendar capacity. A person will confirm the practical date and time separately. The optional ICS file is only a local reminder generated on your device." : "Dit reserveert geen capaciteit in een agenda van AIOW of een leverancier. Een mens bevestigt datum en tijd apart. Het optionele ICS-bestand is alleen een lokale herinnering die op je apparaat wordt gemaakt."}</p><button type="button" className={styles.primaryButton} onClick={() => downloadIcs(form, requestId, locale)}>{en ? "Download local reminder (.ics)" : "Download lokale herinnering (.ics)"}</button></div> : <form onSubmit={submit}>
        ...
        {step === 1 && <div className={styles.formFields}><label>{en ? "Subject" : "Onderwerp"}<select id="booking-subject" value={form.subject} aria-invalid={Boolean(errors.subject)} aria-describedby={errors.subject ? "booking-subject-error" : undefined} onChange={(e) => update("subject", e.target.value)}><option value="bedrijf">{en ? "AI for business" : "AI voor bedrijf"}</option><option value="pand">Smart Office</option><option value="woning">Home</option><option value="anders">{en ? "Other" : "Anders"}</option></select>{errors.subject && <small id="booking-subject-error">{errors.subject}</small>}</label><label>{en ? "Context" : "Toelichting"}<textarea id="booking-details" maxLength={1200} rows={4} required value={form.details} aria-invalid={Boolean(errors.details)} aria-describedby={errors.details ? "booking-details-error" : undefined} onChange={(e) => update("details", e.target.value)} />{errors.details && <small id="booking-details-error">{errors.details}</small>}</label></div>}
        {step === 2 && <div className={styles.formFields}><label>{en ? "Date" : "Datum"}<input id="booking-date" type="date" min={amsterdamDateISO(new Date(), 1)} required value={form.date} aria-invalid={Boolean(errors.date)} aria-describedby={errors.date ? "booking-date-error" : undefined} onChange={(e) => update("date", e.target.value)} />{errors.date && <small id="booking-date-error">{errors.date}</small>}</label><fieldset tabIndex={errors.slot ? -1 : undefined} aria-invalid={Boolean(errors.slot)} aria-describedby={errors.slot ? "booking-slot-error" : undefined}><legend>{en ? "Time (Europe/Amsterdam)" : "Tijd (Europe/Amsterdam)"}</legend><div className={styles.slots}>{BOOKING_SLOTS.map((slot) => <button type="button" key={slot} aria-pressed={form.slot === slot} onClick={() => update("slot", slot)}>{slot}</button>)}</div>{errors.slot && <small id="booking-slot-error">{errors.slot}</small>}</fieldset></div>}
