...
import { scanHrefForPillar, scanSubjectForPillar } from "@/lib/aiow-v1/scan-intent";
...
export function PillarPage({ data, locale = "nl" }: { data: Pillar; locale?: AiowLocale }) {
  ...
  const initialSubject = scanSubjectForPillar(data.slug);
  const scanHref = scanHrefForPillar(data.slug, locale);
  const bookingTrigger = useRef<HTMLElement | null>(null);
  function openBooking(event: MouseEvent<HTMLAnchorElement>) { event.preventDefault(); bookingTrigger.current = event.currentTarget; setBooking(true); }
  return <div className={`${shell.site} ${styles.pillarSite}`} data-pillar={data.slug}><PublicHeader locale={locale} variant="human-industrial" scanHref={scanHref} /><main>
    ...
    <header className={styles.hero}><p className={shared.eyebrow}>{data.eyebrow}</p><h1>{data.title}</h1><div className={styles.mobileScan}><Link href={scanHref} className={`${shared.primaryButton} ${styles.scanButton}`} onClick={openBooking}>{en ? "Request a scan" : "Vraag een scan aan"}</Link><p>{en ? "Free · about 30 minutes · outcome: a decision memo. A person confirms the date and time." : "Gratis · circa 30 minuten · uitkomst: een beslismemo. Een mens bevestigt datum en tijd."}</p></div><p className={styles.answer}>{data.answer}</p><p className={styles.intro}>{data.introduction}</p></header>
    ...
    <section className={styles.cta}><h2>{en ? "Start with one real situation." : "Begin met één echte situatie."}</h2><p>{en ? "Bring a workflow, a data question or a space. We clarify what is feasible now, which dependencies exist and what belongs outside the scope." : "Neem een werkstroom, gegevensvraag of ruimte mee. We maken duidelijk wat nu haalbaar is, welke afhankelijkheden bestaan en wat buiten scope hoort."}</p><Link href={scanHref} className={`${shared.primaryButton} ${styles.scanButton}`} onClick={openBooking}>{en ? "Request a scan" : "Vraag een scan aan"}</Link></section>
  </main><PublicFooter locale={locale} /><BookingModal open={booking} onClose={() => setBooking(false)} locale={locale} returnFocus={bookingTrigger.current} initialSubject={initialSubject} /></div>;
}
