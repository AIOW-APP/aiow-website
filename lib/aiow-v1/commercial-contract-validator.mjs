...
function outboxOrderKey(item) {
  return [item.nextAttemptAt === null ? "" : item.nextAttemptAt, item.createdAt, item.id];
}
...
export function validateOutboxBatchAckV1(ack, { operation, requestedLimit } = {}) {
  ...
  for (let index = 1; index < ack.items.length; index += 1) {
    if (compareTuple(outboxOrderKey(ack.items[index - 1]), outboxOrderKey(ack.items[index])) >= 0) return false;
  }
  return true;
}
