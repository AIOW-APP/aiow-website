...
export function buildQuoteMailContent({ quoteNumber, snapshot, contact, source, country = "", receivedAt }) {
  ...
  const customerText = [
    ...
    AIOW_COMPANY.legalName,
    `${AIOW_COMPANY.streetAddress} · ${AIOW_COMPANY.postalCode} ${AIOW_COMPANY.locality}`,
    `KvK ${AIOW_COMPANY.chamberOfCommerce} · ${AIOW_COMPANY.publicEmail} · ${AIOW_COMPANY.website}`,
  ].join("\n");
  ...
  return freeze({
    customerMail: { from: AIOW_COMPANY.transactionalEmail, to: [contact.email], subject: customerSubject, text: customerText, html: textToSafeHtml(customerText, snapshot.bookingUrl, en ? "en" : "nl") },
    internalMail: { from: AIOW_COMPANY.transactionalEmail, to: [AIOW_COMPANY.transactionalEmail], subject: internalSubject, text: internalText, html: textToSafeHtml(internalText) },
  });
}

export function escapeHtml(value) { return String(value).replace(/[&<>"']/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[character]); }
function textToSafeHtml(value, ctaUrl, locale = "nl") {
  const body = escapeHtml(value).replace(/\n/g, "<br>\n");
  const safeUrl = ctaUrl ? escapeHtml(ctaUrl) : "";
  const ctaLabel = locale === "en" ? "OPEN THE AIOW SCAN" : "OPEN DE AIOW-SCAN";
  const brandLabel = locale === "en" ? "CUSTOM AI · HOOFDDORP" : "MAATWERK-AI · HOOFDDORP";
  const serviceLabel = locale === "en" ? "DESIGN · BUILD · CONNECT · MANAGE" : "ONTWERP · BOUW · KOPPEL · BEHEER";
  const authorityLabel = locale === "en" ? "A person decides. Always." : "Een mens beslist. Altijd.";
  const cta = safeUrl ? `<tr><td style="padding:24px 0 0"><a href="${safeUrl}" style="display:inline-block;padding:14px 20px;background:#d94b30;color:#11110f;text-decoration:none;font-weight:800;border:1px solid #d94b30">${ctaLabel}</a></td></tr>` : "";
  return `<!doctype html><html lang="${locale}"><body style="margin:0;padding:0;background:#d6d8d2;color:#11110f;font-family:Arial,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;background:#d6d8d2"><tr><td align="center" style="padding:24px 12px"><table role="presentation" width="640" cellspacing="0" cellpadding="0" border="0" data-aiow-brand="human-industrial" style="width:100%;max-width:640px;background:#f5f4ee;border:1px solid #9fa09a"><tr><td style="padding:24px 28px;background:#11110f;color:#f5f4ee;font-size:22px;font-weight:800;letter-spacing:2px">AIOW <span style="color:#ffb5a3">●</span></td></tr><tr><td style="padding:30px 28px 34px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td style="padding:0 0 12px;color:#8b2418;font-size:12px;font-weight:800;letter-spacing:2px">${brandLabel}</td></tr><tr><td style="border-top:4px solid #d94b30;padding:22px 0 0;font-size:16px;line-height:1.65">${body}</td></tr>${cta}<tr><td style="padding:30px 0 0;color:#50514c;font-size:12px;line-height:1.5">${serviceLabel}<br>${authorityLabel}</td></tr></table></td></tr></table></td></tr></table></body></html>`;
}
