...
import contract from "../../docs/contracts/aiow-commercial-control-plane-v1.json" with { type: "json" };
...
const CANONICAL_ENDPOINTS = new Set(["booking", "analytics", "ops_mutation", "quote_prepare"]);
export function endpointPayloadDigest(endpoint, value) {
  if (!CANONICAL_ENDPOINTS.has(endpoint) || !plain(value)) throw new TypeError("invalid canonical endpoint payload");
...
export async function commercialRpc(name,args,{env=process.env,fetchImpl=fetch,timeoutMs=8000}={}){
  ...
  const payload=await boundedResponseJson(response); if(!response.ok){const pg=plain(payload)&&typeof payload.code==="string"?payload.code:""; const conflict=["P0001","23505","40001"].includes(pg)||response.status===409; throw new CommercialError(pg==="40001"?"revision_conflict":conflict?"idempotency_conflict":"invalid_request",conflict?409:(response.status===400?400:502),"RPC rejected",payload);} return payload;
}
export function revisionConflictFromRpc(error) {
  if (!(error instanceof CommercialError) || error.code !== "revision_conflict" || !plain(error.payload)) return null;
  const detail = error.payload.details;
  try { const value = typeof detail === "string" ? JSON.parse(detail) : detail; return validateContractDefinition("RevisionConflict", value) ? value : null; } catch { return null; }
}
