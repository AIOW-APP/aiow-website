...
export function AiowV1Homepage({ locale = "nl" }: { locale?: "nl" | "en" }) {
  ...
  return <div className={styles.site}><PublicHeader locale={locale} onBook={openBooking} />
    <main>
      ...
      <section id={en ? "solutions" : "oplossingen"} className={styles.solutions}><div className={styles.sectionHead}><p className={styles.eyebrow}>{en ? "Solutions" : "Oplossingen"}</p><h2>{en ? "Four routes. One measured way of working." : "Vier routes. Eén beheerste werkwijze."}</h2><p>{en ? "Choose the environment where AI must become useful. Every route starts with boundaries and a real implementation scope." : "Kies de omgeving waarin AI nuttig moet worden. Elke route begint bij grenzen en een echte implementatiescope."}</p></div><div className={styles.solutionList}>{solutions.map(([title, body, href], index) => <Link href={href} key={title} className={styles.solution}><span>0{index + 1}</span><div><h3>{title}</h3><p>{body}</p></div><b aria-hidden="true">↗</b></Link>)}</div></section>
    ...
    </main><PublicFooter locale={locale} showYear /><BookingModal open={booking} onClose={() => setBooking(false)} locale={locale} returnFocus={bookingTrigger.current} /><QuoteModal open={quote !== null} onClose={() => setQuote(null)} locale={locale} returnFocus={quoteTrigger.current} calculatorConfig={quote} />
  </div>;
}
