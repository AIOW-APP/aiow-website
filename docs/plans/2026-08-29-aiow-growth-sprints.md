...
**First action:** close the live fail-open venture admin route with host-first 404 and application-level Basic authentication before any durable data migration or activation. No commercial table, outbox, provider, analytics or retention activation may precede that closure.

### Executable commercial-control-plane v1 freeze

The Draft 2020-12 authority at `docs/contracts/aiow-commercial-control-plane-v1.json` is normative; prose below is a release checklist, not an alternate contract.

- The root is a closed discriminated union. Every booking/quote request, prepare/commit ACK/error, lead/delivery/queue projection, ops mutation/ACK/conflict, analytics event/ACK/report, provider-neutral mail job/result, outbox, provider gate, audit and retention dry-run result is recursively closed with explicit types, formats, nullability, bounds and enums.
- HTTP is frozen at `POST /api/booking`, `POST /api/quote`, `POST /api/events`, `GET /api/ops/leads`, `PATCH /api/ops/leads/{leadId}`, `GET /api/ops/report`, `POST /api/internal/booking-adapter`, `POST /api/internal/quote-adapter` and `POST /api/internal/mail-outbox/run`, including exact auth, bodies, headers, statuses and schema references.
- SQL compatibility retains `aiow_quote_prepare_v1` and `aiow_quote_commit_v1`; the successor adds `aiow_booking_commit_v1`, `aiow_commercial_queue_v1`, `aiow_commercial_mutate_v1`, `aiow_commercial_report_v1`, `aiow_commercial_event_v1`, all six `aiow_mail_outbox_*_v2` finalize/claim/resolve authorities, and `aiow_commercial_retention_dry_run_v1` with exact argument and ACK references.
- The migration is append-only after immutable `20260828_aiow_quote_adapter_v1.sql`: common `commercial_leads`, source-specific `booking_leads`, `commercial_mail_outbox` v2, raw/daily commercial events, audit, provider gates and endpoint-scoped idempotency. Quote rows backfill one-to-one through `commercial_lead_id`; old quote tables/RPCs remain compatibility authorities; v1 outbox receives no new jobs after v2 activation.
- Every public or operator mutation uses endpoint + key identity, normalized canonical fields, RFC 8785 canonical JSON SHA-256 and an atomically stored original outcome. Same key/digest replays before revision checks with no residue; same key/different digest is 409; a non-replay stale expected revision returns the exact current projection and revision with 409.
- Lifecycle edges are exactly `new → qualified|awaiting_info|scan_planned|lost`, `qualified → awaiting_info|scan_planned|proposal|lost`, `awaiting_info → qualified|scan_planned|lost`, `scan_planned → proposal|won|lost`, `proposal → won|lost`, `won` terminal, and authenticated reasoned `lost → qualified` reopen only. Accepted non-replays increment once, stamp configured actor/server time and add one audit row. Mail, providers, analytics and clients cannot transition leads; unread only clears; priority stays enum-bound; terminal next action is null.
- Ops host recognition uses a platform-verified hostname and exact server allowlist; custom production domains return 404 before auth/data. The allowed deployment host then requires TLS Basic auth using exact configured username/strong secret and the configured principal with `ops_admin`; missing/malformed config fails closed, query/URL credentials and fallbacks are forbidden, unauthenticated is 401 plus `WWW-Authenticate`, unauthorized is 403, and queue/report/PII/mutate/outbox-resolve/legal-hold capabilities cover UI, APIs, exports and PII assets. Vercel SSO is an outer additional gate only.
- Booking creates exactly `customer_booking` + `internal_booking`; quote creates exactly `customer_quote` + `internal_lead`; `(commercial_lead_id, kind)` is unique. Closed immutable payload hashes, seven states, five-minute token leases, CAS revisions, five attempts and accepted/transient-pre-acceptance/permanent-pre-acceptance/ambiguous provider outcomes are authoritative. Graph HTTP 202 requires acceptance kind and attempt receipt while provider message ID may be null. Ambiguity enters review and never auto-resends; only authenticated idempotent revision-bound, reasoned, audited manual resolution may mark sent with evidence, approve resend or dead-letter.
- Provider execution is disabled by default. Production permits Microsoft Graph only; Gmail is legacy preview/test-only and forbidden in production. Exact tenant/application/mailbox/sender, secret presence, Graph `Mail.Send` application role, Exchange mailbox-restriction readback, evidence digest, owner approval, expiry/currentness and capability binding are required with no fallback. Activation remains a separate external owner gate.
- Quote prepare durably creates quote/common-lead state but stays out of queue, unread, SLA and analytics. Commit atomically stores PDF, both v2 jobs and committed state. Prepare/commit replay is immutable; 24-hour orphans expire with PII redaction; only the committed ACK permits PDF delivery.
- Analytics is a closed per-event union with canonical route/context/value enums and explicit `experiment_exposed`; conversions bind to the same server-validated experiment/variant. UUID event identity deduplicates; changed digest conflicts. Raw IP, user-agent, referrer, query, UTM and free text are neither persisted nor logged. Reports are closed daily aggregates only.
- Retention starts dry-run only. Lead PII, PDFs and outbox payloads redact/delete 90 days after terminal/abandoned anchor unless active customer relation or audited `ops_admin` legal hold; raw analytics is 30 days, aggregates 13 months, provider receipts 90 days, and non-PII audit facts 365 days after irreversible redaction. Destructive activation is a separate owner gate.
